...
Проект следует спецификации [Conventional Commits 1.0.0](https://www.conventionalcommits.org/en/v1.0.0/).
...
- **`<type>`** — обязательный тип коммита (существительное в нижнем регистре)
- **`[optional scope]`** — опциональная область кода в скобках
- **`!`** — перед `:` указывает на BREAKING CHANGE
- **`<description>`** — краткое описание изменений
- **`[optional body]`** — детальное описание (отделяется пустой строкой)
- **`[optional footer(s)]`** — футеры, например `BREAKING CHANGE:` или `Refs: #123`
...
| `style` | Форматирование, стиль кода (не логика) | — |
| `refactor` | Рефакторинг без исправлений и фич | — |
| `perf` | Оптимизация производительности | — |
| `test` | Добавление или исправление тестов | — |
| `build` | Система сборки, зависимости | — |
| `ci` | CI/CD конфигурация | — |
| `chore` | Рутинные задачи, не относящиеся к коду | — |
| `revert` | Откат изменений | — |

### BREAKING CHANGE

Breaking change **обязательно** обозначается одним из двух способов:

1. `!` в заголовке: `feat(api)!: изменить формат ответа`
2. Футером: `BREAKING CHANGE: поле \`name\` удалено из ответа`
...
```
feat: добавить маршрут POST /api/bookings

```
```
fix(calendar): исправить расчёт часового пояса для UTC+5

При бронировании слотов в часовом поясе UTC+5 время смещалось на 1 час.

Closes: #42
```
```
feat(api)!: переименовать resourceId → eventTypeId

BREAKING CHANGE: поле resourceId заменено на eventTypeId во всех эндпоинтах
```
```
chore(deps): обновить Vuetify до последней версии
```

### Revert

```
revert: откатить фикс часового пояса
